...
public class EntityExistsException extends RuntimeException {
	...
	public EntityExistsException(Class clazz, String... searchParamsMap) {
		super(EntityExistsException.generateMessage(clazz.getSimpleName(), toMap(searchParamsMap)));
	}
	...
	private static <K, V> Map<K, V> toMap(Object... entries) {
		...
		return IntStream
			...
			.collect(
				HashMap::new,
				(m, i) ->
					m.put(((Class<K>) String.class).cast(entries[i]), ((Class<V>) String.class).cast(entries[i + 1])),
				Map::putAll
			);
	}
}
