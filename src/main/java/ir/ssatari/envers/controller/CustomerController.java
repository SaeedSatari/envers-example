...
public class CustomerController {
	...
	public CustomerResponseList getCustomerList() {
		...
		customerResponseList.setCustomerResponses(
			customers.stream().map(CustomerMapper.MAPPER::customerEntityToCustomerResponse).collect(Collectors.toList())
		);
	...
	public CustomerResponse saveNewCustomer(@RequestBody CustomerRequest customerRequest) {
		...
		CustomerEntity customerEntity = customerService.saveNewCustomer(
			CustomerMapper.MAPPER.customerRequestToCustomerEntity(customerRequest)
		);
	...
	public CustomerHistoryResponseList getHistory(@PathVariable String id) {
		...
		responseList.setCustomerHistoryResponses(
			histories
				.stream()
				.map(CustomerMapper.MAPPER::customerHistoryEntityToCustomerHistoryResponse)
